...
for pk in pokemon_array
    pokemon_dict[pk.name.toLowerCase()] = pk.types
...
for type, cnt of type_count
    type_count_array.push(type: type, cnt: cnt) if cnt > 0
        ...
        handlers:
            "!typegame": enabler.enabled (command) ->
                ...
                if cache.running
                    ...
                    i = 0
                    while type
                        types.push(type_list[i]) if type & 1
                        type >>= 1
                        ++i
                    "Name #{cache.cnt} Pokémon with the type #{types.join('/')}!"
            ...
            "!scramble": enabler.enabled (command) ->
                ...
                if cache.running
                    ...
                    "Unscramble this name: #{_.shuffle(cache.name).join(" ")}"

            "!guess": enabler.enabled (command) ->
                cache = typegameCache[command.channel] = typegameCache[command.channel] || {}
                if cache.running
                    if command.args[0]?
                        type = pokemon_dict[command.args[0].toLowerCase()]
                        if type?
                            type = _.reduce(_.map(type, (t) -> 1 << types[t]), (a, b) -> a | b)
                            if type&cache.type == cache.type
                                unless cache.guessed[command.args[0]]
                                    cache.guessed[command.args[0]] = true
                                    cnt = cache.userCount[command.nickname] = cache.userCount[command.nickname] + 1 || 1;
                                    if cnt == cache.cnt
                                        cache.running = false
                                        "Correct! #{command.nickname} wins this round!"
                                    else
                                        "Correct!"
                            else
                                "Sorry, #{command.args[0]} has different types."
                else
            ...
            "!unscramble": enabler.enabled (command) ->
                ...
                if cache.running
                    if command.args[0]?
                        if command.args[0].toLowerCase() == cache.name.toLowerCase()
                            cache.running = false
                            "Correct! #{command.nickname} wins this round!"
                        else
