...
for pk in pokemon_array
    pokemon_dict[pk.name.toLowerCase()] = pk
    type_count[1<<types[pk.types[0]]] += 1
    if pk.types.length > 1
        type_count[1<<types[pk.types[1]]] += 1
        type_count[(1<<types[pk.types[0]])|(1<<types[pk.types[1]])] += 1

type_count_array = []
for type, cnt of type_count
    type_count_array.push(type: parseInt(type), cnt: cnt) if cnt > 0

module.exports =
    init: (client, imports) ->
        leaderKey = client.config("command-trigger")
        enabler = imports.enable.getEnabler("typegame")

        typegameCache = {}
        scrambleCache = {}
        runningCache  = {}

        guessType = (channel, nick, guess) ->
            cache = typegameCache[channel] = typegameCache[channel] || {}
            guess = guess.toLowerCase()
            if cache.running
                pk = pokemon_dict[guess]
                if pk?
                    type = _.reduce(_.map(pk.types, (t) -> 1 << types[t]), (a, b) -> a | b)
                    if type&~cache.type && type&cache.type && (type|cache.type)==type || type==cache.type
                        unless cache.guessed[guess]
                        ...
                        "Sorry, #{pk.name} has the type #{pk.types.join("/")}."
            else
                "Sorry, a game is not currently running."

        guessScramble = (channel, nick, guess) ->
            cache = scrambleCache[channel] = scrambleCache[channel] || {}
            if cache.running
        ...
        handlers:
            "!typegame": enabler.enabled (command) ->
                cache = typegameCache[command.channel] = typegameCache[command.channel] || {}
                if cache.running
                    "A game is still running! Name #{cache.cnt} Pokémon with the type #{cache.types.join('/')}!"
                else
                    ...
                    i = 0
                    while type
                        cache.types.push(type_list[i]) if type & 1
                        type >>= 1
                        ++i
                    "Name #{cache.cnt} Pokémon with the type #{cache.types.join('/')}!"

            "!scramble": enabler.enabled (command) ->
                cache = scrambleCache[command.channel] = scrambleCache[command.channel] || {}
                if cache.running
                    "A game is still running! Unscramble this name: #{cache.scrambled}"
                else
