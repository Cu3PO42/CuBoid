...
module CuBoid.Typegame.Data {
    export var pokemon_array = [
        ...
        { name: 'Mega Venasaur', types: ['Grass','Poison']},
    ...
    export var pokemon_dict: {[name: string]: { name: string, types: string[] }} = {}
    for (let i = 0; i < pokemon_array.length; i++) {
        let pk = pokemon_array[i];
        pokemon_dict[pk.name.toLowerCase()] = pk;
        type_count[1<<types[pk.types[0]]]++;
        if (pk.types.length > 1) {
            type_count[1<<types[pk.types[1]]]++;
            type_count[(1<<types[pk.types[0]])|(1<<types[pk.types[1]])]++;
        }
    }
